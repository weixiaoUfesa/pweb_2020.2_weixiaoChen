...
def processar_arquivo(ano):
    print(f'开始{ano}')
    arquivo = f'E:/dengue/dengue_dados/DENGBR{ano}.csv'
        ...
        with tqdm(total=total_chunks, desc=f'Lendo {ano}', leave=False) as pbar:
            for chunk in pd.read_csv(arquivo, chunksize=chunksize, low_memory=False):
                try:
                    ...
                    print(f'partes_filtradas.append')
...
if __name__ == "__main__":
    anos = [24]  # de 2019 a 2023
    print(f'{anos}')
    with Pool(processes=4) as pool:
        # 用 imap_unordered 可以边处理边显示进度
        for _ in tqdm(pool.imap_unordered(processar_arquivo, anos), total=len(anos), desc='Arquivos'):print(f'ok')
